// Typing-session Night mode smoke (local only; NOT wired into CI).
//
// Prereqs: web dev server on :5173 (guest onboarding courses are local).
// Run: node apps/web/scripts/night-mode-probe.mjs
//
// Uses the stable onboarding guest course id (no API JWT required).
...
async function main() {
  ...
  // N1: clear override + light system → night off
  ...
  // N3: force off while system dark
  ...
  const storedOff = await page.evaluate((key) => localStorage.getItem(key), NIGHT_KEY);
  assert(storedOff === '0', 'N3: storage is 0');
  await page.emulateMedia({ colorScheme: 'light' });
  ...
  const storedCleared = await page.evaluate((key) => localStorage.getItem(key), NIGHT_KEY);
  assert(storedCleared === null, 'N4: storage cleared');
  await page.waitForFunction(() => document.documentElement.classList.contains('dark'));
  ...
  const storedOn = await page.evaluate((key) => localStorage.getItem(key), NIGHT_KEY);
  assert(storedOn === '1', 'N5: storage is 1');

  // N6: timer-end dialog under night (portal + html.dark). Guests have no leave blocker.
  ...
  // N7: leave typing → no dark on list
  await page.evaluate((key) => localStorage.setItem(key, '1'), NIGHT_KEY);
  await page.goto(`${WEB}/courses/short`, { waitUntil: 'networkidle' });
  assert(!(await htmlHasDark(page)), 'N7: list page must not keep html.dark');
  assert(
